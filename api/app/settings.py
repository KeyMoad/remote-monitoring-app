"""
SRE Agent Settings File

***Do not change any variable name in this file***

You can change the any setting you want in this file by changing the value of variables
"""
...
APP_DESCRIPTION = config("APP_DESCRIPTION", default="An API to make jobs easier.\n\n\t- ## Admin\n\tA tag that contains **admin** level routes.\n\n\t- ## Auth\n\tJust a few routes for **Authentication and Authorization**.\n\n\t- ## User\n\tEverything a user needs to have fun with the application.", cast=str)
# license informations
LICENSE_INFO = config("LICENSE_INFO", default={
                      "name": "Develop, Design, and Support by LimooHost SRE Team.", "url": "https://limoo.host"}, cast=dict)


# Settings
LOG_PATH = config("LOG_PATH", default="../app.log", cast=str)
DEBUG = config("DEBUG", default=False, cast=bool)
VERSION = config("VERSION", cast=str)
QUEUE_PATH = config("QUEUE_PATH", default="../.app_jobs.lock", cast=str)
CONTROL_PANEL = config(
    "CONTROL_PANEL", default=server_controlpanel(), cast=str)
SERVER_HOSTNME = config("SERVER_HOSTNME", default=server_hostname(), cast=str)
SERVER_IP = config("SERVER_IP", default=server_ip(), cast=str)
ALLOW_ORIGINS = config("ALLOW_ORIGINS", default=["localhost"], cast=list)
